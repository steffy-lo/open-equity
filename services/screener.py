...
services.screener
=================
Canonical screener implementation for Open Equity.
Supports watchlist screens, off-watchlist custom universes, metadata-aware
signal ingestion, and signal retrieval filters.
"""
import json
import logging
import os
...
from config import MIN_SIGNAL_CONFIDENCE, WATCHLIST_PATH
...
def _score(price_data: dict, tech: dict) -> tuple:
    ...
    div_yield_raw = price_data.get("div_yield")
    div_yield = div_yield_raw or 0.0
    ...
    if fcf is not None and fcf < 0 and (price_data.get("sector") or "") != "Financial Services":
        flags.append("Negative free cash flow — burning through cash")
    if debt_to_equity > 350 and (price_data.get("sector") or "") != "Financial Services":
    ...
    if pct_52w >= -1.5:
        ...
        confidence += 0.10
        reasons.append("Within 10% of 52w high")
    ...
    if div_yield and 0 < div_yield <= 0.05:
        confidence += 0.02
        reasons.append(f"Sustainable dividend yield ({div_yield*100:.1f}%)")
...
def _apply_scan_mode(result: dict, mode: str | None) -> dict:
    if not mode:
        return result

    updated = dict(result)
    confidence = float(updated.get("confidence") or 0.0)
    rsi = updated.get("rsi_14") or 50.0
    pct_52w = updated.get("pct_from_52w_high") or -50.0
    vol_ratio = updated.get("volume_ratio") or 1.0
    pe = updated.get("pe")
    reasons = [updated.get("reason") or ""]

    if mode == "breakout":
        if -6 <= pct_52w <= -0.2:
            confidence += 0.08
            reasons.append("Mode breakout, near high with room to break")
        if vol_ratio >= 1.2:
            confidence += 0.05
            reasons.append("Mode breakout, supporting volume")
        if rsi > 78:
            confidence -= 0.18
            reasons.append("Mode breakout penalty, too extended")

    elif mode == "pullback":
        if -25 <= pct_52w <= -8:
            confidence += 0.10
            reasons.append("Mode pullback, retraced from highs into a reset zone")
        if 40 <= rsi <= 58:
            confidence += 0.08
            reasons.append("Mode pullback, RSI reset with rebuilding momentum")
        if rsi > 75:
            confidence -= 0.15
            reasons.append("Mode pullback penalty, still too hot")

    elif mode == "quality_value_momentum":
        if pe and 10 <= pe <= 30:
            confidence += 0.10
            reasons.append("Mode QVM, valuation in preferred range")
        if 50 <= rsi <= 68:
            confidence += 0.06
            reasons.append("Mode QVM, balanced momentum")
        if pct_52w < -35:
            confidence -= 0.08
            reasons.append("Mode QVM penalty, trend damage too large")

    confidence = max(0.0, min(round(confidence, 2), 1.0))
    if updated.get("signal") != "flag":
        updated["signal"] = "buy" if confidence >= MIN_SIGNAL_CONFIDENCE else "neutral"
    updated["confidence"] = confidence
    updated["reason"] = " | ".join([r for r in reasons if r])
    updated["scan_mode"] = mode
    return updated
...
def run_screen(
    ...
    for t in normalized_tickers:
        try:
            ...
            result = _apply_scan_mode(result, scan_mode)
